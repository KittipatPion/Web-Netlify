...
const fieldMain = [
  ...
  {
    key: "haulageShipmentStatus",
    label: "Haulage Shipment Status",
  },
  {
    key: "shippingShipmentStatus",
    label: "Shipping Shipment Status",
  },
  {
    key: "paymentStatus",
...
const PaymentExport = () => {
  ...
  const [isSelectImportFile, setIsSelectImportFile] = useState(false);
  const [arrSearchData, setArrSearchData] = useState([]);
  const [typeSelectImportFile, setTypeSelectImportFile] = useState("");
  ...
  const [isSelectTransporterId, setIsSelectTransporterId] = useState(false);
  const [selectBox, setSelectBox] = useState(false);
  const [uploadOrDownloadCount, setUploadOrDownloadCount] = useState(10);
  const [isCantCreatePayment, setIsCantCreatePayment] = useState(false);
  const [errorSH, setErrorSH] = useState("");
  const [isLoadingDataDetail, setIsLoadingDataDetail] = useState(false);
  const [isSameShipmentCreate, setIsSameShipmentCreate] = useState(false);
  ...
  const onSelectTransporter = (id, type) => {
    setIsSelectTransporterId(!isSelectTransporterId);
    var arrSelect = [...selectItemsSearch];
    var setNewSelectitems = baseItems.filter((x) => x.isChecked === true);

    var isErrorSame = false;

    if (arrSelect[1].transporterId) {
      var objSameCreate = setNewSelectitems.filter(
        (x) =>
          x.transporterId === arrSelect[1].transporterId &&
          x.haulageShipmentStatus !== null &&
          x.haulageShipmentStatus !== ""
      );
      if (objSameCreate.length) {
        isErrorSame = true;
        setIsSameShipmentCreate(!isSameShipmentCreate);
      }
    } else if (arrSelect[5].transporterId) {
      var objSameCreate = setNewSelectitems.filter(
        (x) =>
          x.shippingId === arrSelect[5].transporterId &&
          x.shippingShipmentStatus !== null &&
          x.shippingShipmentStatus !== ""
      );
      if (objSameCreate.length) {
        isErrorSame = true;
        setIsSameShipmentCreate(!isSameShipmentCreate);
    ...
    if (!isErrorSame) {
      if (type === "shipping") {
        setNewSelectitems.filter(
          (x) =>
            x.shippingShipmentStatus === "" || x.shippingShipmentStatus === null
        );
        if (setNewSelectitems) {
          fnGetCompanyListByMain();
          fnGetShipmentDataForPayment(setNewSelectitems, id);
          setCostData();
          fnGetCompanyCost();
          ...
          setErrorSH("shipping");
          setIsCantCreatePayment(!isCantCreatePayment);
        }
      } else if (type === "hualage") {
        setNewSelectitems.filter(
          (x) =>
            x.haulageShipmentStatus === "" || x.haulageShipmentStatus === null
        );
        if (setNewSelectitems) {
          fnGetCompanyListByMain();
          fnGetShipmentDataForPayment(setNewSelectitems, id);
  ...
  const handleClickOpen = (type, contractId) => (e) => {
    if (type === "add") {
      var setNewSelectitems = [];
      var arrSelect = [...selectItemsSearch];
      console.log(
        "baseItems",
        baseItems.filter((x) => x.isChecked === true)
      );

      var isErrorSame = false;

      if (arrSelect[1].transporterId) {
        setNewSelectitems = baseItems.filter(
          (x) =>
            x.isChecked === true &&
            (x.haulageShipmentStatus === "" || x.haulageShipmentStatus === null)
        );
        ...
        console.log("objSameCreate", objSameCreate);
        if (objSameCreate.length) {
          isErrorSame = true;
          setIsSameShipmentCreate(!isSameShipmentCreate);
        }
      ...
      if (!isErrorSame) {
        var id = arrSelect[1].transporterId
          ? arrSelect[1].transporterId
          : arrSelect[5].transporterId
          ? arrSelect[5].transporterId
          : _TranspoterId;
        console.log(setNewSelectitems);
        console.log("setNewSelectitems", setNewSelectitems);
        console.log("arrSelect[1]", arrSelect[1]);
        console.log("selectItemsSearch[4]", selectItemsSearch[4]);
        if (arrSelect[1].transporterId && !arrSelect[5].transporterId) {
        ...
        } else if (!arrSelect[1].transporterId && arrSelect[5].transporterId) {
          if (baseItems[0] && setNewSelectitems.length) {
            console.log("2");
            fnGetCompanyListByMain();
            fnGetShipmentDataForPayment(setNewSelectitems, id);
  ...
  const onClickExportPDF = () => {
    ...
    // Render to View PDF
    FunctionController.renderPDF(newDoc, true);

    // console.log(newDoc);

    // Convert PDF to File in Variable
    FunctionController.convertPDFToFile(newDoc, "Cover Sheet", true).then(
      (result) => {
        console.log(result);
      }
    );
  ...
  const collapseHeader = (type) => {
    ...
    if (type === "add") {
      if (companyList[0] && (shipmentData.length || extrachargeData.length)) {
        var transporter = selectItemsSearch[1].transporterId
          ? selectItemsSearch[1]
          : selectItemsSearch[5];
        var DueDate =
          transporter.dueDate !== null ? new Date(transporter.dueDate) : null;
                            ...
                            >
                              <CInput
                                ...
                                onChange={(e) => {
                                  ...
                                  if (selectItemsSearch[1].transporterId) {
                                    newArr[1].dueDate = e.target.value;
                                  } else {
                                    newArr[5].dueDate = e.target.value;
                                  }

                                  setSelectItemsSearch(newArr);
                                  // selectItemsSearch[1].dueDate = e.target.value
                                }}
                                placeholder="date"
                                required
                              />
  ...
  const onClickCheckCreatePayment = () => {
    ...
    if (
      ...
      getIsValidForm("invoice-info-need-validation") &&
      ...
      if (exportModeId == 2 || exportModeId == 3 || exportModeId == 4) {
        setIsSelectImportFile(!isSelectImportFile);
      } else {
        setTypeSelectImportFile("Delivery");
        setIsConfirmSave(!isConfirmSave);
      }
    } else {
  ...
  const onClickSelectImportFile = (type) => {
    setTypeSelectImportFile(type);
    setIsSelectImportFile(!isSelectImportFile);
    setIsConfirmSave(!isConfirmSave);
  };
  ...
  const onClickCreatePayment = () => {
    ...
    costItems.map((z) => {
      if (z.costElement !== "" && z.costCenter) {
        ...
        invoiceItems.map((x, indexmain) => {
          ...
          // console.log("checkbox", x.mpaymentWhtRates[indexmain]);
          if (indexmain < 3) {
    ...
    newObj.exportModeId = exportModeId;
    ...
    newObj.actualVat = parseInt(acVatsum);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (i === 2) {
        ...
        if (typeSelectImportFile === "Delivery") {
          newList.fileData = FunctionController.convertDataToFile(
            fieldMain,
            FunctionController.setNumberValueInArray(
              fieldMain,
        ...
        } else if (typeSelectImportFile === "SaleOder") {
          newList.fileData = FunctionController.convertDataToFile(
            fieldsSaleOrderList,
            setGroupBySaleOrder(setNewSelectitems),
            "SaleOrderList"
          );

          newListSup.fileName = "SaleOrderList.xlsx";
        }
        setTypeSelectImportFile("");
    ...
    fnCreatePayment(newObj, newFileList, shipmentIOList, isExport);
  };

  const setGroupBySaleOrder = (data = []) => {
    var arrData = [
      ...FunctionController.setNumberValueInArray(fieldMain, data),
    ];
    var newArr = [];
    // console.log(arrData);
    ...
    arrData.map((data) => {
      ...
      if (!newArr.length) {
        ...
        var index = newArr.findIndex((x) => x.saleOrderNo == data.saleOrderNo);
        if (index != -1) {
  ...
  // const onHandleChangeVatInvoice = (type, index) => (e) => {
  //   var newArr = [...invoiceItems];
  //   var sum = 0;
  //   var wht = 0;
  //   newArr.map((x, index) => {
  ...
  //   if (type === "vat") {
  //     newArr[index].mpaymentVatRates.checkbox = e.target.checked;
  //     if (e.target.checked === false) {
  //       // setTotalPrice(sum);
  //     } else {
  //       // setTotalPrice(sum + newArr[index].mpaymentVatRates.vatAmount);
  //     }
  //   }

  //   if (type === "wht") {
  //     newArr[index].mpaymentWhtRates[0].checkbox = e.target.checked;
  //     if (e.target.checked === false) {
  //       // setTotalPrice(sum);
  //     } else {
  //       // setTotalPrice(sum + newArr[index].mpaymentWhtRates.vatAmount);
  //     }
  //   }
  //   // if(type === "baseAmount"){
  ...
  const onHandleChangeVat = (type) => (e) => {
    ...
    var sum = 0;
    var wht = 0;
    console.log("invoiceItems", newArr);
    newArr.map((x, index) => {
      if (index < 3) {
        wht = wht + x.mpaymentWhtRates[index].vatAmount;
      }
  ...
  const onHandleChangeWhtRateNew = (type, id) => (e) => {
    ...
    if (type === "baseAmount") {
      ...
      if (parseInt(e.target.value) <= newArr[indexFiles[0]].totalAmount) {
        newArr[indexFiles[0]].mpaymentWhtRates[indexItem].baseAmount = parseInt(
          e.target.value
        );
        // newArr[indexFiles[0]].mpaymentWhtRates[0].baseAmount2 = parseInt(e.target.value);
        newArr[indexFiles[0]].mpaymentWhtRates[indexItem].vatAmount =
  ...
  const onClickCheckSearchData = () => {
    ...
    // }
    //   search-startend-need-validation

    // Object.keys(newArr[1]).length && Object.keys(newArr[4]).length &&
    if (
      getIsValidForm("search-startend-need-validation") &&
      (newArr[1].transporterId || newArr[5].transporterId) &&
      newArr[4].length
    ) {
      ...
      var arrError = [true, true];
      if (newArr[1].transporterId || newArr[5].transporterId) {
        arrError[0] = false;
      }
      if (newArr[4].length) {
        arrError[1] = false;
      }
      setInvalidMaterialFormSearch(arrError);
      setBaseItems([]);
    }
  };
  ...
  const onClickSearchData = () => {
    ...
    var shippingid = getHasObjectValue(arrSelect[5])
      ? arrSelect[5].transporterId
      : null;
    ...
    deliveryMode = !isNaN(deliveryMode) ? deliveryMode : null;
  ...
  const fnGetDataList = (arrData = []) => {
    setIsPostingData(true);
    Repository.fetchGetExpDeliveryListForCreatePayment(arrData).then(
      (result) => {
        setIsPostingData(false);
        // //console.log()
        if (result.httpCode === "200") {
          ...
          if (result.data.vtexpshipmentList) {
            setBaseItems(
  ...
  const fnGetDataListById = (arrData = []) => {
    ...
    Repository.fetchGetExpDeliveryListById(arrData).then(
      (result) => {
        ...
        if (result.httpCode === "200") {
          console.log("EXPDATA", result.data);
  ...
  const fnGetShipmentDataForPayment = (arrData = [], id) => {
    console.log(arrData);
    console.log(id);
    setIsPostingData(true);

    Repository.fetchGetExpShipmentDataForPayment(arrData, id).then(
      (result) => {
        setIsPostingData(false);
        if (result.httpCode === "200") {
          console.log(result.data);
  ...
  const fnGetCompanyListByMain = () => {
    ...
    var id = null;
    if (Object.keys(selectItemsSearch[1]).length) {
      id = selectItemsSearch[1].transporterId;
    } else {
      id = selectItemsSearch[5].transporterId;
    }
    Repository.fetchTransporterListByMain(id).then(
      (result) => {
        setIsLoaded(true);
        if (result.httpCode === "200") {
  ...
  const formDialogTabOne = () => {
    if (Object.keys(items).length) {
      console.log("saleOrderPriceList", transportRateList);
      var newObj = { ...items };
      if (!isShipmentNonSO) {
        ...
        <CDataTable
          ...
          scopedSlots={{
            ...
            checked: (item, index) => {
              ...
              if (
                Object.keys(newArr[1]).length &&
                Object.keys(newArr[5]).length
              ) {
                if (
                  newArr[5].transporterId == item.shippingId &&
                  newArr[1].transporterId == item.transporterId
                ) {
                  if (
                    item.shippingShipmentStatus == "" &&
                    item.haulageShipmentStatus == ""
                  ) {
                    isPass = true;
                  }
                }
                ...
                if (newArr[1].transporterId == item.haulageId) {
                  if (item.haulageShipmentStatus == "") {
                    isPass = true;
                  }
                }
              } else if (
                ...
                if (newArr[5].transporterId == item.shippingId) {
                  if (item.shippingShipmentStatus == "") {
                    isPass = true;
                  }
                }
              } else {
              ...
              if (
                item.edpstatus !== "Mismatch" &&
                (item.shipmentStatus === "Open" ||
                  item.shipmentStatus === "Reopen") &&
            ...
            shippingShipmentStatus: (item) => (
              <td>
                <h4>
                  <CBadge color={getBadge(item.shippingShipmentStatus)}>
                    {item.shippingShipmentStatus}
                  </CBadge>
                </h4>
              </td>
            ),
            haulageShipmentStatus: (item) => (
              <td>
                <h4>
                  <CBadge color={getBadge(item.haulageShipmentStatus)}>
                    {item.haulageShipmentStatus}
                  </CBadge>
                </h4>
              </td>
  ...
  const modalForm = () => (
    <div>
      ...
      {/* End Edit Status Modal */}
      <CModal
        show={isCantCreatePayment}
        onClose={() => setIsCantCreatePayment(!isCantCreatePayment)}
        color="warning"
        centered
      >
        <CModalHeader closeButton>
        ...
        <CModalBody>เอกสารการวางบิลของ{errorSH}ถูกสร้างแล้ว</CModalBody>
        <CModalFooter>
          <CButton
            color="success"
            onClick={() => setIsCantCreatePayment(!isCantCreatePayment)}
          >
            {Constant.btOK}
          </CButton>{" "}
          {/* <CButton
      ...
      <CModal
        show={isSelectTransporterId}
        // onClose={() => setIsShowFormChangePass(!isShowFormChangePass)}
        // style={{display : "none"}}
        size="lg"
        closeButton={true}
        color="success"
      ...
      >
        <CModalHeader closeButton>
          <h5>
            <CModalTitle>เลือก Vender ที่ต้องการจะวางบิล</CModalTitle>
          </h5>
        </CModalHeader>
        <CModalBody>
                ...
                <CFormGroup>
                  ...
                  >
                    <CInput
                      ...
                      value={
                        "[" +
                        selectItemsSearch[1].transporterCode +
                        "]" +
                        selectItemsSearch[1].transporterNameThai
                      }
                    />
                  ...
                  <Checkbox
                    id="check-transportId"
                    checked={!selectBox}
                    onChange={(e) => setSelectBox(!e.target.checked)}
                    // defaultChecked={item.isChecked}
                    color="primary"
                  />
                </CFormGroup>
                ...
                <CFormGroup>
                  ...
                  >
                    <CInput
                      value={
                        "[" +
                        selectItemsSearch[5].haulageCode +
                        "]" +
                        selectItemsSearch[5].transporterNameThai
                      }
                    />
        ...
        <CModalFooter>
          <CButton
            color="success"
            onClick={(e) => {
              var arr = [...selectItemsSearch];
              var id = null;
              var type = "";
              if (selectBox === true) {
                id = arr[5].transporterId;
                type = "shipping";
              } else {
                id = arr[1].transporterId;
                type = "hualage";
              }
              onSelectTransporter(id, type);
            }}
          >
            {Constant.btOK}
          </CButton>{" "}
          <CButton
            color="secondary"
            onClick={() => setIsSelectTransporterId(!isSelectTransporterId)}
          >
            {Constant.btCancel}
          </CButton>
        </CModalFooter>
      ...
      {/* Start Error Same Shipment Create Modal */}
      <CModal
        show={isSameShipmentCreate}
        onClose={() => setIsSameShipmentCreate(!isSameShipmentCreate)}
        color="warning"
        centered
      >
        <CModalHeader closeButton>
        ...
        <CModalBody>
          {Constant.contentWarningSameShipmentCreatePayment}
        </CModalBody>
        <CModalFooter>
          <CButton
            color="secondary"
            onClick={() => setIsSameShipmentCreate(!isSameShipmentCreate)}
          >
            {Constant.btOK}
          </CButton>
        </CModalFooter>
      </CModal>
      {/* End Error Same Shipment Create Modal */}
    </div>
  );
  ...
  const modalDialog = () => (
    <div>
      ...
      >
        ...
        <CModalBody>{Constant.contentConfirmCreatePayment}</CModalBody>
        <CModalFooter>
          <CButton color="success" onClick={onClickCreatePayment}>
            {Constant.btOK}
          </CButton>{" "}
          <CButton
            color="secondary"
      ...
      {/* Start Select Import File Modal */}
      <CModal
        show={isSelectImportFile}
        onClose={() => setIsSelectImportFile(!isSelectImportFile)}
        color="success"
        centered
      >
        <CModalHeader closeButton>
        ...
        <CModalBody>
          {Constant.contentSelectImportFilePayment}
          <br />
          <br />
          <CRow>
            <CCol>
              <CFormGroup>
                <SiMicrosoftexcel
                  size="30px"
                  color="green"
                  onClick={() => onClickSelectImportFile("Delivery")}
                />{" "}
            ...
            <CCol>
              <CFormGroup>
                <SiMicrosoftexcel
                  size="30px"
                  color="green"
                  onClick={() => onClickSelectImportFile("SaleOder")}
                />{" "}
