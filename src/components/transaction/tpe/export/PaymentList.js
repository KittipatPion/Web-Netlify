...
const PaymentViewExport = () => {
  ...
  const [isOpenAttractFile, setIsOpenAttractFile] = useState(false);
  ...
  const [setIsConfirmsManualDoc, isConfirmsManualDoc] = useState(false);
  ...
  const handleClickOpen = (type, paymentId) => (e) => {
    if (type === "add") {
      ...
      setTestShow(true);
      setOpenAddForm(true);
      fnGetDataPaymentListById(paymentId);
      ...
      return (
        <CCard className="fixed d-flex">
          <CCardBody>
            ...
            <CRow className="justify-content-end">
              <CCol xs="12" sm="6" md="4" className="pl-3 pr-3 text-right">
                <CLabel style={{ color: "black", fontSize: "20px" }}>
            ...
            <CRow className="justify-content-end">
              ...
              {roleUser !== trstCode ? (
                <CCol md="4">
                  <CRow>
                    <CCol xs="12" sm="6" md="6" className="mr-autopl-3 pr-3 ">
                      <CButton
                        size={Constant.btAddSize}
                        onClick={() =>
            ...
            <CRow className="justify-content-end">
              ...
              {/* <CCol xs="12" sm="6" md="2" className=" pr-3 pt-2">
                <CButton
                  size={Constant.btAddSize}
                  onClick={() => setIsOpenAttractFile(!isOpenAttractFile)}
                  className={"btn-mainsmp"}
                  style={{ fontSize: "14px" }}
                  block
                >
  ...
  const onClickExportData = (type = '') => (e)  => {
    var setNewSelectitems = baseItems;
    ...
    if (type === "add") {
      if (Object.keys(editData).length) {
        ...
        if (
          !costItems.length &&
          !invoiceItems.length &&
          newShowData.tpaymentItems.length
        ) {
          console.log(newShowData.tpaymentItems);
          newShowData.tpaymentItems.map((x, index) => {
          ...
          console.log("newtpaymentDoc", newtpaymentDoc);
        }

        if (!supportfile.length && newShowData.tpaymentSupportFiles.length) {
  ...
  const fnCancelPayment = (index) => {
    setIsPostingData(true);
    Repository.fetchCancelPaymentById(index, true).then(
      (result) => {
        setIsPostingData(false);
        if (result.httpCode === "200") {
  ...
  const onClickUpdateCreatePayment = () => {
    ...
    costItems.map((z) => {
      ...
      console.log("Crete invoiceItems", invoiceItems);
      invoiceItems.map((x, indexmain) => {
        ...
        console.log("Crete tpaymentAttachFiles", x);
  ...
  const sumtotalprice = () => {
    ...
    invoiceItems.map((x) => {
      ...
      if (x.documentTypeId === "05") {
        sum = sum - y.totalAmount;
      } else {
        sum = sum + y.totalAmount;
        console.log("x.totalAmount", y.totalAmount);
      }
    });
  ...
  const onClickSearchData = () => {
    ...
    var userId = _UserId;
    var userTransporterId = _TranspoterId;
    var type = "";
    var saleOrderNo = "";
  ...
  const fnGetDataList = (arrData = [], type) => {
    setIsPostingData(true);
    Repository.fetchGetExpDeliveryListForCreatePaymentListView(
      arrData,
      type
    ).then(
      (result) => {
  ...
  const fnSendToAllPay = (paymentId) => {
    ...
    setIsVatNotEqual(false);
    // setIsPostingData(true);
    Repository.fetchSendToAllPay(paymentId).then(
      (result) => {
        setIsPostingData(false);
        // //console.log()
        if (result.httpCode === "200") {
  ...
  const fnGetDataPaymentListById = (paymentId) => {
    setIsPostingData(true);
    Repository.fetchGetCreatPaymentById(paymentId, true).then(
      (result) => {
        setIsPostingData(false);
                ...
                <CRow className="ml-2 mr-2 p-0">
                  ...
                  <CCol xs="12" sm="6" md="3">
                    <CFormGroup>
                      ...
                      >
                        <CSelect id="search-deliveryMode" value={1} disabled required>
                          <option selected hidden value="">
                            {Constant.txtformPlaceholderSelected}
                          </option>
  ...
  const modalForm = () => (
    <div className="c-body">
      ...
      >
        ...
        <CModalBody>
          จำนวน VAT ไม่เท่ากัน ยืนยันที่จะทำรายการหรือไม่ ?
        </CModalBody>
        <CModalFooter>
          <CButton
            color="secondary"
      ...
      {/* Start Manual Doc Modal */}

      <CModal
        show={isConfirmsManualDoc}
        onClose={() => setIsConfirmsManualDoc(!isConfirmsManualDoc)}
        color="success"
        centered
      >
        <CModalHeader closeButton>
        ...
        <CModalBody>ต้องการแนบใบปะหน้าใหม่ใช่หรือไม่ ?</CModalBody>
        <CModalFooter>
          <CButton color="success" onClick={fnCreateDocManual}>
            {Constant.btOK}
          </CButton>
          <CButton
            color="secondary"
      ...
      {/* End Manual Doc Modal */}

      {/* Start Manual Doc Modal */}

      <CModal
        show={isOpenAttractFile}
        onClose={() => setIsOpenAttractFile(!isOpenAttractFile)}
        color="success"
        centered
      >
        <CModalHeader closeButton>
          <h5>
            <CModalTitle>เอกสารแนบไฟล์</CModalTitle>
          </h5>
        </CModalHeader>
        <CModalBody>
          <CRow xs="12" sm="12" md="12">
            <CCol xs="12" sm="6" md="4" className=" pr-3 pt-2">
              <CButton
                size={Constant.btAddSize}
                onClick={onClickExportPDF}
                className={"btn-mainsmp"}
            ...
            <CCol xs="12" sm="6" md="4" className=" pr-3 pt-2">
              <CButton
                size={Constant.btAddSize}
                onClick={onClickExportData('shipment')}
                className={"btn-mainsmp"}
                style={{ fontSize: "14px" }}
                block
              >
            ...
            <CCol xs="12" sm="6" md="4" className=" pr-3 pt-2">
              <CButton
                size={Constant.btAddSize}
                onClick={onClickExportData('delivery')}
                className={"btn-mainsmp"}
                style={{ fontSize: "14px" }}
                block
              >
...
export default PaymentViewExport;
