...
const ApproveCreditDebit = () => {
  ...
  const onClickSearchData = () => {
    ...
    // var isTransfer = document.getElementById("search-workType").value;
  ...
  const fnGetExtraChargeNameOnlyList = () => {
    if (!extraChargeNameList.length) {
      Repository.fetchGetExtraChargeNameOnlyList().then(
        (result) => {
          if (result.httpCode === "200") {
            var filterData = result.data.filter((x) => x.extraChargeCode.includes('DTransfer'))
  ...
  const fnRejectExtraChargeStatusList = (arrData = []) => {
    setIsPostingData(true);
    Repository.fetchTransferRejectCreditDebitStatusList(arrData).then(
      (result) => {
        if (result.httpCode === "200") {
          fnSetValueinHistoryLog(arrData);
        } else {
                    ...
                    <CRow>
                      <CCol xs="6" sm="8" md="10">
                        <h3 className="headtext">
                          {Constant.txtTransactionInitial}
                        </h3>
                      </CCol>
                      <CCol xs="6" sm="4" md="2">
                        {/* {dialogs("show")} */}
